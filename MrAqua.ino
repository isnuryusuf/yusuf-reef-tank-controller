...
const int pwrLight2Pin = A1; //arduino A1
const int pwrFilterPin = A2; //arduino A2
const int pwrCircPin = A3; //arduino A3
...
//yusuf
const int dosingPump4 = 2; //led1Pin
const int dosingPump5 = 3; //led2Pin
const int dosingPump6 = 4; //led3Pin
const int dosingPump7 = 5; //led4Pin
const int dosingPump8 = 6; //led5Pin
//yusuf
...
// Pin 48 for watchdog - see: watchdogSetup
#define ONE_WIRE_BUS_H 49  //Heatsink sensor on pin 49

//yusuf change from MCP9701AT-E/TT to DS18B20 
//#define ONE_WIRE_BUS_I A13  //Internal sensor on pin A13
...
//yusuf change from MCP9701AT-E/TT to DS18B20 
//OneWire oneWireI(ONE_WIRE_BUS_I);
...
//yusuf change from MCP9701AT-E/TT to DS18B20 
//DallasTemperature sensorI(&oneWireI);

// Device address variables
DeviceAddress waterSensor;
DeviceAddress heatsinkSensor;

//yusuf //yusuf change from MCP9701AT-E/TT to DS18B20 
//DeviceAddress internalSensor;
...
boolean sensor2Enabled = true; // heatsink
boolean sensor3Enabled = true; // internal
boolean displaySensor1 = true;
boolean displaySensor2 = true;
boolean displaySensor3 = true;
...
float sensor1Calibration = 0.1; // calibration offset for sensor 1 (water)
float sensor2Calibration = 0.1; // calibration offset for sensor 2 (heatsink)
float sensor3Calibration = 0.1; // calibration offset for sensor 3 (internal)
boolean fan1status = false;
boolean fan2status = false;
byte internalFanOnTemp = 23; //yusuf, default 45
byte internalFanOffTemp = 20; //yusuf, default 40
byte heatsinkFanOnTemp = 23; //yusuf, default 40
byte heatsinkFanOffTemp = 20; //yusuf, default 45
...
//byte heatOffTemp = 30; //def 30, Chiller OffTemp
//byte heatOnTemp = 27;  //def 25, Chiller OnTemp
byte heatOffTemp = 22;
byte heatOnTemp = 27;
...
//yusuf
boolean pump4State = false;
boolean pump5State = false;
boolean pump6State = false;
boolean pump7State = false;
boolean pump8State = false;
//yusuf
...
//yusuf
unsigned long pump4StartMillis = 0;
unsigned long pump5StartMillis = 0;
unsigned long pump6StartMillis = 0;
unsigned long pump7StartMillis = 0;
unsigned long pump8StartMillis = 0;
//yusuf
...
//yusuf
unsigned long pump4millis = 0;
unsigned long pump5millis = 0;
unsigned long pump6millis = 0;
unsigned long pump7millis = 0;
unsigned long pump8millis = 0;
//yusuf


//ATO variables
boolean ATOEnabled = true;
boolean ResSwitchEnabled = true; //default false
boolean WaterLevel = HIGH;
boolean ReservoirLevel = HIGH;
boolean ATOAlarm = true; //default false
...
void setup()
{
  ...
  //Serial3.println("HM10 serial started at 115200");
  ...
  myGLCD.print(F("YHS v1.0.1"), CENTER, 292); //yusuf
  //Serial.println(F("v2.5.3"));
  Serial.println(F("YHS v1.0.1")); //yusuf
  ...
  // used for PWM lighting control //yusuf-disable
  ...
  //yusuf
  //pinMode(dosingPump4, OUTPUT);
  //pinMode(dosingPump5, OUTPUT);
  //pinMode(dosingPump6, OUTPUT);
  //pinMode(dosingPump7, OUTPUT);
  //pinMode(dosingPump8, OUTPUT);
  //yusuf
  ...
  digitalWrite(dosingPump5,LOW);
  digitalWrite(dosingPump6,LOW);
  digitalWrite(dosingPump7,LOW);
  digitalWrite(dosingPump8,LOW);
  //yusuf
  ...
  //yusuf //yusuf change from MCP9701AT-E/TT to DS18B20 
  //sensorI.begin();                       //start up temperature library
  //sensorI.getAddress(internalSensor, 0); //get addresses of temperature sensor  
...
void loop()
{
  ...
  if(pump4State)
  {
    if(currentMillis > pump4StartMillis + pump4millis)
    {
      pump4State = false;
      analogWrite(dosingPump4, 0);
      Serial.print(F("Pump4 done.\n"));
      if(dispScreen == 10)myFiles.load(92, 170, 48, 48, "10test.raw",2);
    }
  }
  if(pump5State)
  {
    if(currentMillis > pump5StartMillis + pump5millis)
    {
      pump5State = false;
      analogWrite(dosingPump5, 0);
      Serial.print(F("Pump5 done.\n"));
      if(dispScreen == 10)myFiles.load(92, 170, 48, 48, "10test.raw",2);
    }
  }
  if(pump6State)
  {
    if(currentMillis > pump6StartMillis + pump6millis)
    {
      pump6State = false;
      analogWrite(dosingPump6, 0);
      Serial.print(F("Pump6 done.\n"));
      if(dispScreen == 10)myFiles.load(92, 170, 48, 48, "10test.raw",2);
    }
  }
  if(pump7State)
  {
    if(currentMillis > pump7StartMillis + pump7millis)
    {
      pump7State = false;
      analogWrite(dosingPump7, 0);
      Serial.print(F("Pump7 done.\n"));
      if(dispScreen == 10)myFiles.load(92, 170, 48, 48, "10test.raw",2);
    }
  }
  if(pump8State)
  {
    if(currentMillis > pump8StartMillis + pump8millis)
    {
      pump8State = false;
      analogWrite(dosingPump8, 0);
      Serial.print(F("Pump8 done.\n"));
      if(dispScreen == 10)myFiles.load(92, 170, 48, 48, "10test.raw",2);
    }
  }
  //yusuf
  ...
  //{
  //  data = Serial3.read();Serial3.print(data);    //Pembacaan dan ditampilkan data yang masuk   
  //  Serial3.print("\n"); 
    //Data yang masuk      
  //  if(data == '1'){
  ...
  //  Serial3.write(appData);
  //}
  //if (Serial3.available()) {           // Read user input if available.
  //  delay(10);
  //  HM10.write(Serial3.read());
  //}
  //if ( inData == "F") {
  //  Serial3.println("LED OFF");
  //  digitalWrite(13, LOW); // switch OFF LED
  //  delay(500);
  //}
  //if ( inData == "N") {
  //  Serial3.println("LED ON");
...
void drawATO()
{
  ...
  {
    //yusuf
    //myFiles.load(122, 45, 115, 45, "logo-gio.raw",2); //display a iAqua logo if ATO is disabled
...
//yusuf - this code has change from heater mode to chiller mode
void checkTemp() // called from the main loop to update temperature reading
{
  if(sensor1Enabled)
  {
    if(sensorW.isConnected(waterSensor))
    {
      ...
      {
        if (intTemperature <= 22)  // turn off heater if over threshold
        {
          Serial.print(F("Water below temp threshold! turn off chiller.\n"));
          Serial.print(F("Sensor Raw temp: ")); //yusuf temp
          Serial.println(sensorW.getTempC(waterSensor)); //yusuf temp
          Serial.print(F("Current temp: ")); //yusuf temp
          Serial.print(intTemperature); //yusuf temp
          Serial.print(F("\n"));
          Serial.print(F("Setting off chiller temp: ")); //yusuf temp
          Serial.print(heatOffTemp); //yusuf temp
          Serial.print(F("\n"));
          Serial.print(F("Setting on chiller temp: ")); //yusuf temp
          Serial.print(heatOnTemp); //yusuf temp
          Serial.print(F("\n"));
      ...
      else if (heaterWarning == true) // if there was a warning, we watch for a low temp to turn heat back on
      {
        ...
        {
          Serial.print(F("Water temp over threshold,  turn off chiller.\n"));
          Serial.print(F("Sensor Raw temp: ")); //yusuf temp
          Serial.println(sensorW.getTempC(waterSensor)); //yusuf temp
          Serial.print(F("Current temp: ")); //yusuf temp
          Serial.print(intTemperature); //yusuf temp
  ...
  if(sensor3Enabled)
  {
    ...
    if(displayInC == false) temperature3 = sensorI.getTempF(internalSensor);
    if(displayInC == true)  temperature3 = sensorI.getTempC(internalSensor);
    temperature3 = temperature3 + sensor3Calibration;
    int intTemperature3 = temperature3;
    */
...
void drawTemp()
{
  ...
  if(displayMultiSensors == false) //only water temp is enabled
  {
    if(heaterWarning)
    {
      myFiles.load(30, 35, 60, 51, "1therm.raw",4); //yusuf change from 1thermR
    }else
    {
      myFiles.load(30, 35, 60, 51, "1thermR.raw",4); //yusuf change from 1therm
    }
    if(sensorW.isConnected(waterSensor))
    {
  ...
  else //more than one sensor enabled
  {
    ...
    if(heaterWarning)
    {
      myFiles.load(2, 33, 28, 24, "1thermS.raw",4); //yusuf, change from 1thermRS
    }else
    {
      myFiles.load(2, 33, 28, 24, "1thermRS.raw",4); //yusuf, change from 1thermS
    }
    if(sensorW.isConnected(waterSensor))
    {
      // set temp color based on alarms on home screen
...
void checkDosing() // updates dosing info on the home screen
{
  ...
  //yusuf
  long dose4Cap=EEPROM.read(460);
  dose4Cap=dose4Cap*10;  // to power of 10
  long dose5Cap=EEPROM.read(461);
  dose5Cap=dose5Cap*10;  // to power of 10
  long dose6Cap=EEPROM.read(462);
  dose6Cap=dose6Cap*10;  // to power of 10
  long dose7Cap=EEPROM.read(463);
  dose7Cap=dose7Cap*10;  // to power of 10
  long dose8Cap=EEPROM.read(464);
  dose8Cap=dose8Cap*10;  // to power of 10
  //yusuf
 ...
 // here we prepare to draw the fill over the screen icons
  ...
  if(doses1 > 4)myFiles.load(106, 122, 22, 35, "1ferts2.raw",4);
  if(doses1 < 5)myFiles.load(106, 122, 22, 35, "1fertse2.raw",4);
  if(doses2 > 4)myFiles.load(126, 122, 22, 35, "1ferts2.raw",4);
  if(doses2 < 5)myFiles.load(126, 122, 22, 35, "1fertse2.raw",4);
  if(doses3 > 4)myFiles.load(146, 122, 22, 35, "1ferts2.raw",4);
  if(doses3 < 5)myFiles.load(146, 122, 22, 35, "1fertse2.raw",4);

  //yusuf   xx atas, yy kanan, P, L, gambar, x, y   //x1,y1,x2,y2
  if(doses4 > 4)myFiles.load(106, 162, 22, 35, "1ferts2.raw",4);
  if(doses4 < 5)myFiles.load(106, 162, 22, 35, "1fertse2.raw",4);
  if(doses5 > 4)myFiles.load(126, 162, 22, 35, "1ferts2.raw",4);
  if(doses5 < 5)myFiles.load(126, 162, 22, 35, "1fertse2.raw",4);
  if(doses6 > 4)myFiles.load(146, 162, 22, 35, "1ferts2.raw",4);
  if(doses6 < 5)myFiles.load(146, 162, 22, 35, "1fertse2.raw",4);
  if(doses7 > 4)myFiles.load(106, 202, 22, 35, "1ferts2.raw",4);
  if(doses7 < 5)myFiles.load(106, 202, 22, 35, "1fertse2.raw",4);
  if(doses8 > 4)myFiles.load(126, 202, 22, 35, "1ferts2.raw",4);
  if(doses8 < 5)myFiles.load(126, 202, 22, 35, "1fertse2.raw",4);          
  //yusuf
  ...
  if (doses1>0) y1mac = 163 - ((vol1*39)/dose1Cap);  // for Macros
  if (doses2>0) y1mic = 163 - ((vol2*39)/dose2Cap);  // for Micros
  if (doses3>0) y1exc = 163 - ((vol3*39)/dose3Cap);  // for Excel

  //yusuf
  if (doses4>0) y1pump4 = 203 - ((vol4*39)/dose4Cap);  // for Pump4
  if (doses5>0) y1pump5 = 203 - ((vol5*39)/dose5Cap);  // for Pump5
  if (doses6>0) y1pump6 = 203 - ((vol6*39)/dose6Cap);  // for Pump6
  
  if (doses7>0) y1pump7 = 243 - ((vol7*39)/dose7Cap);  // for Pump7
  if (doses8>0) y1pump8 = 243 - ((vol8*39)/dose8Cap);  // for Pump8
  //yusuf
  ...
  myGLCD.setColor(0, 0, 0);
  myGLCD.fillRect(111,125,122,y1mac); //x1,y1,x2,y2
  myGLCD.fillRect(131,125,142,y1mic); 
  myGLCD.fillRect(151,125,162,y1exc); 

  //yusuf
  myGLCD.fillRect(111,165,122,y1pump4); 
  myGLCD.fillRect(131,165,142,y1pump5); 
  myGLCD.fillRect(151,165,162,y1pump6); 
  
  myGLCD.fillRect(111,205,122,y1pump7); 
  myGLCD.fillRect(131,205,142,y1pump8);     
  //yusuf
  ...
  myGLCD.print(char3, xcharExcel, 112);

  //yusuf
  itoa(doses4, char3, 10);
  myGLCD.print(char3, xcharPump4, 154);
  itoa(doses5, char3, 10);
  myGLCD.print(char3, xcharPump5, 154);
  itoa(doses6, char3, 10);
  myGLCD.print(char3, xcharPump6, 154);
  itoa(doses7, char3, 10);
  myGLCD.print(char3, xcharPump7, 194);
  itoa(doses8, char3, 10);
  myGLCD.print(char3, xcharPump8, 194);
  //yusuf
}
