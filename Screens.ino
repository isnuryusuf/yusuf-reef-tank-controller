...
void screenHeater() // screen for choosing temp settings for the heater
{
  ...
  myGLCD.setFont(arial_bold);
  myGLCD.setColor(34, 81, 255);
  myGLCD.print(F("CHILLER SETUP"), 36, 12);
...
void screenDosing(byte pumpNo)  // screen to configure the 3 dosing pumps
{
  ...
  {
    pumpTime=EEPROM.read(455); // 34 // pump 4 sec/ml
  }
  else if (pumpNo==5)
  {
    pumpTime=EEPROM.read(456); // 35 // pump 5 sec/ml
  }
  else if (pumpNo==6)
  {
    pumpTime=EEPROM.read(457); // 36 // pump 6 sec/ml
  }
  else if (pumpNo==7)
  {
    pumpTime=EEPROM.read(458); // 37 // pump 7 sec/ml
  }
  else if (pumpNo==8)
  {
    pumpTime=EEPROM.read(459); // 38 // pump 8 sec/ml
  }
  //yusuf
  ...
  if (pumpNo==4)
  {
    doseCap=EEPROM.read(460); // 39 // dosing 4 reservior capacity in mL;    
  }
  if (pumpNo==5)
  {
    doseCap=EEPROM.read(461); // 40 // dosing 5 reservior capacity in mL;    
  }
  if (pumpNo==6)
  {
...
void screenDosingSched(byte pumpNo) 
{
  ...
  else if (pumpNo==4)
  {
    ...
    myGLCD.print(F("Pump 4"), 85, 70);
    onHour=pump4.onHour;
    onMin=pump4.onMinute;
    daySun=pump4.Sunday;
    dayMon=pump4.Monday;
    dayTue=pump4.Tuesday;
    dayWed=pump4.Wednesday;
    dayThu=pump4.Thursday;
    dayFri=pump4.Friday;
    daySat=pump4.Saturday;
  }
  else if (pumpNo==5)
  {
    myGLCD.setColor(255, 255, 0);  // yellow for pump5
    myGLCD.print(F("Pump 5"), 85, 70);
    onHour=pump5.onHour;
    onMin=pump5.onMinute;
    daySun=pump5.Sunday;
    dayMon=pump5.Monday;
    dayTue=pump5.Tuesday;
    dayWed=pump5.Wednesday;
    dayThu=pump5.Thursday;
    dayFri=pump5.Friday;
    daySat=pump5.Saturday;
  }
  else if (pumpNo==6)
  {
    myGLCD.setColor(255, 0, 255);  // purple for pump6
    myGLCD.print(F("Pump 6"), 85, 70);
    onHour=pump6.onHour;
    onMin=pump6.onMinute;
    daySun=pump6.Sunday;
    dayMon=pump6.Monday;
    dayTue=pump6.Tuesday;
    dayWed=pump6.Wednesday;
    dayThu=pump6.Thursday;
    dayFri=pump6.Friday;
    daySat=pump6.Saturday;
  }
  else if (pumpNo==7)
  {
    myGLCD.setColor(0, 228, 255);  // cyan for pump7
    myGLCD.print(F("Pump 7"), 85, 70);
    onHour=pump7.onHour;
    onMin=pump7.onMinute;
    daySun=pump7.Sunday;
    dayMon=pump7.Monday;
    dayTue=pump7.Tuesday;
    dayWed=pump7.Wednesday;
    dayThu=pump7.Thursday;
    dayFri=pump7.Friday;
    daySat=pump7.Saturday;
  }
  else if (pumpNo==8)
  {
    myGLCD.setColor(0, 255, 0);  // green for pump8
    myGLCD.print(F("Pump 8"), 85, 70);
    onHour=pump8.onHour;
    onMin=pump8.onMinute;
    daySun=pump8.Sunday;
    dayMon=pump8.Monday;
    dayTue=pump8.Tuesday;
    dayWed=pump8.Wednesday;
    dayThu=pump8.Thursday;
    dayFri=pump8.Friday;
    daySat=pump8.Saturday;
  }
  //yusuf
